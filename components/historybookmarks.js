...
AutoCompleteSearch.prototype =
	...
	startSearch: function(searchString, searchParam, previousResult, listener)
		...
		if(this.result.results&&searchString==(this.result.searchString+searchString[this.result.searchString.length]))
			...
			// HISTORY
			var historySource=RDF.GetDataSource("rdf:history");
			var historyRoot=RDF.GetResource("NC:HistoryRoot");
			...
			while (bookmarks.hasMoreElements())
				...
				if(bookmarkURL&&bookmarkName)
					...
					if(this.execSearch(bookmarkURL, searchString))
						...
						this.result.styles.push('font-weight:bold;');
						}
					}
				}
			bookmarksSource.endUpdateBatch();
			// GET HISTORY
			historySource.beginUpdateBatch();
			var historyItems = historySource.GetTargets(historyRoot, RDFChild, true);
			while (historyItems.hasMoreElements())
				{
				var historyItem = historyItems.getNext();
				historyItem.QueryInterface(Components.interfaces.nsIRDFResource);
				var historyURL = historyItem.Value;
				var historyName = '';
				if(historySource.hasArcOut(historyItem, NameType))
					{
					historyName = historySource.GetTarget(historyItem, NameType, true);
					historyName = historyName.QueryInterface(Components.interfaces.nsIRDFLiteral).Value;
					}
				if(this.execSearch(historyURL, searchString))
					{
					this.result.results.push(historyURL);
					this.result.comments.push(historyName);
					this.result.styles.push(false);
					}
				}
			historySource.endUpdateBatch();
			}
